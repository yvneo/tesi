...
report_home_to_home = train_and_evaluate_model(X_home_filtered, y_home_filtered, X_home_filtered, y_home_filtered, scenario_name="Home_vs_Home", taxonomy_label="ID")
collect_results(report_home_to_home, location_name="Home", label="ID")
report_home_to_home.to_csv("risultati/report_home_to_home.csv", index=False)

#2. addestramento su Home, test su UniVR
report_home_to_univr = train_and_evaluate_model(X_home_filtered, y_home_filtered, X_univr_filtered, y_univr_filtered, scenario_name="Home_vs_UniVR", taxonomy_label="OOD")
collect_results(report_home_to_univr, location_name="UniVR", label="OOD")
report_home_to_univr.to_csv("risultati/report_home_to_univr.csv", index=False)

#3. addestramento su UniVR, test su UniVR
report_univr_to_univr = train_and_evaluate_model(X_univr_filtered, y_univr_filtered, X_univr_filtered, y_univr_filtered, scenario_name="UniVR_vs_UniVR", taxonomy_label="ID")
collect_results(report_univr_to_univr, location_name="UniVR", label="ID")
report_univr_to_univr.to_csv("risultati/report_univr_to_univr.csv", index=False)

#4. addestramento su UniVR, test su Home
report_univr_to_home = train_and_evaluate_model(X_univr_filtered, y_univr_filtered, X_home_filtered, y_home_filtered, scenario_name="UniVR_vs_Home", taxonomy_label="OOD")
collect_results(report_univr_to_home, location_name="Home", label="OOD")
report_univr_to_home.to_csv("risultati/report_univr_to_home.csv", index=False)
...
report_jd_to_univr = train_and_evaluate_model(X_combined, y_combined, X_univr_filtered, y_univr_filtered, scenario_name="Combined_vs_UniVR", taxonomy_label="JD")
collect_results(report_jd_to_univr, location_name="UniVR", label="JD")
report_jd_to_univr.to_csv("risultati/report_jd_to_univr.csv", index=False)

#6. addestramento su dati combinati, test su Home
report_jd_to_home = train_and_evaluate_model(X_combined, y_combined, X_home_filtered, y_home_filtered, scenario_name="Combined_vs_Home", taxonomy_label="JD")
collect_results(report_jd_to_home, location_name="Home", label="JD")
report_jd_to_home.to_csv("risultati/report_jd_to_home.csv", index=False)    
...
plt.savefig("grafici/barplot_finale_tassonomia.png", dpi=300, bbox_inches='tight')
plt.show()